...
MacBook Air M1（16GB）一台で、Rust + CubeCL を限界までチューニングしながらオンデバイスAIを独力開発しています。週20本超の arXiv 論文を実装する生活。限界スペックをハイオク燃料にして駆動中です。
...
**研究・訓練は Python、推論・配布・インフラは Rust**——この二軸で設計しています。
言語を増やすのではなく、役割を明確に切る。摩擦は「何でもRust」でも「何でもPython」でも生まれます。境界を引くことで消えます。
...
**PyTorch 2.7 + Triton 3.x** がスタックの中心です。論文実装・実験・アブレーションはすべて Python で完結させます。`torch.compile(mode="max-autotune")` と BF16 で、クラウド GPU での訓練コストを最小化します。

エコシステムの密度が決定的な差を生みます。arXiv に上がる実装の 9 割は Python です。再現実験のスピードが研究のスピードに直結する以上、訓練フェーズで Python を外す理由はありません。

### Rust — 推論・配布・インフラの主役

訓練済みモデルをオンデバイスで動かすフェーズで Rust が登場します。**GC によるポーズは許容できません**。会話の途中で数百ms の GC ポーズが入れば、製品として死んでいます。Rust は GC を持たない。メモリ安全性はコンパイル時の所有権システムで保証するので、ホットパスにランタイムコストが入りません。

`tch-rs`（PyTorch の Rust バインディング）か `ort`（ONNX Runtime）でモデルをロードし、`extern "C"` で Elixir / Swift / Kotlin すべてから呼べる C-ABI ハブにする。**同じカーネルが iOS でも Android でも動く**のがこの設計の核心です。

**クレート構成:**

- **DL訓練**: 🐍 `PyTorch 2.7`（`torch.compile` + BF16 + Triton カーネル）
- **DL推論**: 🦀 `tch-rs`（PyTorch Rust バインディング）/ `ort`（ONNX Runtime）
- **GPU カーネル**: `CubeCL`——Rust-native GPU カーネル言語。CUDA / Metal / Vulkan を同一の Rust 構文で記述し、バックエンド切り替えがコード変更ゼロ
- **並列**: `rayon` でデータ並列。`.par_iter()` 一枚でスレッドセーフな並列化
- **推論**: `llama-cpp-rs` で LLM/GGUF エッジ推論
- **Web/Backend**: `axum` + `tokio` + `sqlx` で型安全な非同期 API
- **Data**: `polars` で ETL。`bincode` + `zstd` で高圧縮シリアライズ
- **FFI Hub**: `extern "C"` で Elixir / Swift / Kotlin すべてに繋がる

### Elixir / OTP — 分散インフラ

Elixir は分散システムが必要な回にだけ登場します。重い計算は Rust カーネルに全部逃がし、Elixir は「死なないプロセス管理」に専念する役割分担です。

OTP の「let it crash」哲学——`Supervisor` が子プロセスを監視し、障害時に自動再起動します。**深夜3時に落ちても朝には復旧しているシステム**をソロ開発者が一人で維持できる唯一の構成です。

- `GenServer` / `Supervisor` で自己修復プロセスツリー
- `rustler` で Rust NIF を BEAM 上に展開。重い計算は Rust に逃がしつつ、耐障害性は BEAM が担保
- `Phoenix LiveView` でリアルタイムUI（JavaScript をほぼ書かずに済む）

### Swift / Kotlin — ネイティブUI
...
Rust カーネルを外部から呼ぶ FFI には鉄則があります。
...
```
Rust: &[T] ゼロコピースライス → tch::Tensor → ゼロコピー返却
Elixir Binary → rustler NIF → *mut T → kernel → BEAM scheduler
Swift/Kotlin → C-ABI (extern "C") → Rust kernel → ゼロコピー結果
```
...
`extern "C"` でエクスポートした Rust 関数を直接呼びます。Swift は `@_silgen_name`、Kotlin は JNI 経由。カーネルは呼び出し元を知らず、純粋関数として振る舞います。

### 禁止事項

- `Vec` / `Box` の FFI 越境（アロケータが異なるため解放時にクラッシュ）
- 呼び出し元メモリの `Drop`（所有者でないメモリを解放してはいけません）
- FFI 境界をまたぐパニック（`catch_unwind` で必ず境界を張る）
...
M1 の Metal GPU を `CubeCL`（Rust-native GPU カーネル言語）経由で直接叩けば、16GB の統合メモリでも SLM の推論は十分に回ります。`CubeCL` は CUDA / Metal / Vulkan を同一の Rust 構文で記述できるため、バックエンド切り替えがコード変更ゼロで済みます。M1 の統合メモリアーキテクチャでは CPU と GPU がメモリを物理的に共有しているので、ディスクリート GPU のような PCIe を介したデータ転送が不要です。このオーバーヘッドがゼロという特性は、メモリが限られた環境では決定的な利点になります。
...
だから自分の研究は「理論的にエレガントか」よりも「16GB で動くか」が常に最優先の評価軸です。学習だけは `Lambda Labs` / `RunPod` でクラウド GPU を借り、**PyTorch + Triton** で実験を回しますが、推論は必ずオンデバイスに落とします。論文が提案する手法をそのまま使うのではなく、M1 のメモリアーキテクチャに合わせてデータアクセスパターンを最適化し、`tch-rs` / `ort` + `CubeCL` で実行できる形に落とし込む。この「Python 実験 → Rust 推論」の翻訳作業こそが自分の研究の本質だと思っています。
