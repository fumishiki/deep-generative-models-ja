...
:::message
**この記事のゴール**: 自分の業務に1つ以上のAIツールを導入した状態にする。
:::
...
:::details 💡 「業務にAIを入れる」とはどういうことか — 心構え
...
:::message alert
**前提**: この記事は特定のプログラミング知識を前提としません。ビジネスパーソンが自分の業務にAIを導入する実践ガイドです。
:::

:::details 📋 この記事に出てくるツール一覧と料金早見表（2026年2月時点）
...
:::message
**Teams Copilotは最も「ROIが見えやすい」機能**。1時間の会議後に5分で議事録が完成する体験は、一度味わうと手書きには戻れない。
:::
...
:::message alert
**⚠️ Agent Modeの注意点**: 直接ワークブックを編集するので、重要なデータは事前にコピーを取っておくこと。
:::
...
:::message alert
**⚠️ 料金の構造を理解しよう**: M365 Copilotは**単体では契約できない**。まずMicrosoft 365の基本プランに加入した上で、Copilotのライセンスを追加する。
...
:::details 💡 M365 Copilotの効果的なプロンプト例
```
❌ 悪い例: 「売上データを分析して」
...
@[card](https://www.microsoft.com/en-us/microsoft-365-copilot/pricing-new)
...
:::message
**ポイント**: Notion AIは**Businessプラン以上で追加料金なしで使える**。Free/Plusプランでは20回のAI体験版のみ。実質的にAIを使うには**$20/人/月のBusinessプラン**が最低ライン。
:::
...
:::message
**AI × 数式 = 最強の組み合わせ**: Notion AIに「この数式を書いて」と頼むと、自然言語から適切なFormulaを生成してくれる。「期限が3日以内のタスクだけ赤く表示する数式」と言えばAIが書いてくれる。
:::

**Automation（自動化）**: 「ステータスが完了に変更 → 完了日を自動記入 + 担当者にメンション」のようなトリガー→アクションの組み合わせで手作業を排除できる。

:::details 💡 Notion Automationの便利パターン
| パターン | トリガー | アクション | 用途 |
|:---------|:---------|:----------|:-----|
...
@[card](https://www.notion.com/product/ai)
...
:::message
**無料プランでも十分使える**。月に数回プレゼンを作る程度なら無料枠で足りる。社外向けプレゼンにはPro ($18) 以上がおすすめ。
:::
...
:::details 💡 Gamma → PowerPoint ハイブリッドワークフロー
...
@[card](https://workspace.google.com/resources/spreadsheet-ai/)
...
:::message
**AI議事録のコツ**: 会議の冒頭で「今日の議題は○○です」と明確に宣言すると、AIの要約精度が上がる。[S2](./ml-spinoff-02.md)のプロンプト設計と同じ原理 — AIに「文脈」を与えると、出力の質が上がる。
:::
...
:::message
**ポイント**: 自動化は「小さく始める」のが鉄則。まず1つのZap（Zapierの自動化単位）を作って動かし、効果を確認してから広げよう。最初から複雑な自動化を組むと、エラー時の原因特定が困難になる。
:::

@[card](https://zapier.com/)
...
:::message alert
**⚠️ AIに入力したデータは、サービス提供者のサーバーに送信される**
...
:::message
**個人向けプラン（Free/Plus等）の多くは「AIモデルの学習に入力データを使う可能性がある」**と利用規約に書いてある。業務データを扱う場合は必ずBusiness/Enterprise以上のプランを使おう。
:::
...
:::message
**入力前の自問**: 「これは公開できるか？」— 答えがNoなら、匿名化するか、AIに渡さない。
:::
...
:::message
**AI導入の成功法則**: 小さく始める → 効果を測定する → 改善する → 範囲を広げる。一気に全部入れない。
:::
...
:::details 🔬 ロックインを数式で表現する（興味がある人向け）
