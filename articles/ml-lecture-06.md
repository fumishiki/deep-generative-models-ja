---
title: "第6回: 情報理論・最適化理論: 30秒の驚き→数式修行→実装マスター"
emoji: "📡"
type: "tech"
...
Shannon が1948年に打ち立てた情報理論 [^1] は、不確実性を数値化する道具を与えてくれる。Kullback と Leibler が1951年に定義した KL ダイバージェンス [^2] は、分布間の"非対称な距離"を測る。そして Robbins-Monro の確率的近似法 [^3] から始まった最適化の系譜は、Kingma & Ba の Adam [^4] を経て、現代の深層学習の訓練を支えている。

本講義は Course I の第6回 — 数学的武装の総仕上げだ。ここで装備する KL ダイバージェンスは損失関数の設計に、SGD/Adam はパラメータの学習に直結する。この全てが揃って初めて、第7回以降の生成モデルの地図が読める。
...
Perplexity = 2.56 は「モデルが各時点で平均2.56個の選択肢に迷っている」という意味だ。完全な予測なら Perplexity = 1、一様分布（4トークン）なら Perplexity = 4。GPT-4 の Perplexity が低いのは、エントロピーが低い = 不確実性が小さい = よく予測できている、ということに他ならない。

Shannon が1948年の画期的論文 [^1] で定義したこのエントロピーが、2026年の LLM 評価でそのまま使われている。75年以上前の理論が現代 AI の心臓部で生きている。
...
第1回で数式の読み方を学び、第2-3回で線形代数を、第4回で確率論を、第5回で測度論を装備した。本講義はその総仕上げであり、**情報理論と最適化理論という"攻撃手段"**を手に入れる回だ。

ここまでの数学は「世界を記述する道具」だった。線形代数はデータの構造を表し、確率論は不確実性を表し、測度論はそれを厳密にした。だが生成モデルの訓練には、もう2つの道具が要る。

1. **損失関数**: 「モデルがどれだけ悪いか」を数値化する道具 → 情報理論（KL ダイバージェンス）
2. **最適化**: 「モデルをどう改善するか」の手順 → 最適化理論（SGD / Adam）

この2つが揃って初めて、第7回以降の生成モデルの理論が理解できる。
...
松尾研は8回の講義で VAE から Diffusion まで駆け抜ける。だがそれは「数学を知っている人」向けの構成だ。本シリーズは Course I の8回で数学基盤を完全に固め、Course II 以降で理論と実装の両輪を回す。**急がば回れ**。数学の基盤なしに論文は書けない。
...
友人が「明日、太陽が東から昇る」と言っても驚かない（情報量ゼロ）。「明日、太陽が西から昇る」と言えば驚く（情報量大）。エントロピーは「あるイベント群から得られる驚きの平均値」だ。

**メタファー2: KL ダイバージェンス = 翻訳コスト**

$D_\text{KL}(p \| q)$ は「$p$ で生成されたデータを $q$ で符号化したときの余分なビット数」。外国語のテキストを辞書なしで解読するときの追加コストに似ている。$p = q$ なら余分なコストはゼロ。

**メタファー3: 勾配降下 = 霧の中の下山**

パラメータ空間の「谷底」（最適解）に向かって、足元の傾き（勾配）だけを頼りに1歩ずつ下りていく。霧が濃いから谷底は見えない。でも「今の足元が右に傾いているから左に1歩」を繰り返せば、いつか谷底に着く。学習率は「歩幅」だ。大きすぎると谷を飛び越え、小さすぎると日が暮れる。
...
本講義には20以上の数式が登場する。全てを暗記しようとするのは時間の無駄だ。以下の戦略で臨んでほしい。

1. **定義を覚える**: エントロピー、KL、Cross-Entropy の定義式だけは覚える
2. **関係を導出する**: $H(p, q) = H(p) + D_\text{KL}(p \| q)$ は定義から導出する
3. **コードで確認**: 数式を Python で実装して数値が合うか確認する
4. **紙に書く**: 少なくとも KL の非負性証明は紙に書いてみる

:::details トロイの木馬 — Python の遅さが見え始める回
本講義は Python 90% で進む。だが `%timeit` で最適化ループの実行時間を測り始めると「ん？遅くない？」という違和感が生まれる。SGD の for ループは Python が最も苦手とする計算パターンだ。この違和感は第7回で増幅し、第8回の EM 算法で爆発する。そして第9回で Julia が登場し、50倍の高速化を体験することになる。今は静かに `%timeit` の結果を眺めていてほしい。
:::
...
ここからが本番だ。情報理論の核心を全て数式で導出し、最適化理論の主要アルゴリズムをゼロから構築する。紙とペンを用意してほしい。
...
JSD は KL を対称化した距離で、$0 \leq \text{JSD} \leq \log 2$（nat単位）に有界。Goodfellow らの原論文 [^9] では、最適識別器 $D^\star(x) = p_\text{data}(x) / (p_\text{data}(x) + p_g(x))$ のもとで、**生成器**の損失関数が JSD の最小化に帰着することが示されている。

しかし JSD には致命的な弱点がある。$p$ と $q$ のサポートが重ならないとき、JSD は $\log 2$ に飽和して勾配がゼロになる。低次元多様体上に集中する画像分布では、これが頻繁に起きる。
...
$\Pi(\mu, \nu)$ は $\mu$ と $\nu$ を周辺分布に持つ同時分布（**カップリング**）の集合。直感: $\mu$ の砂山を $\nu$ の形に変形するときの**最小輸送コスト**。

これは線形計画問題（LP）であり、3.11b の双対性理論がそのまま適用できる。
...
:::message
ここが本講義の3つの新セクションが繋がる瞬間だ。**凸最適化の強双対性**（3.11b）を**最適輸送の LP**に適用し、双対変数が **1-Lipschitz 関数**（3.11c）に対応する。WGAN（Arjovsky+ 2017 [^13]）はこの KR 双対性を直接利用して、判別器（critic）を 1-Lipschitz 関数として訓練する。
:::
...
def kr_dual_1d(p_samples, q_samples, n_test_funcs=1000):
    """Estimate W1 via KR duality: sup over 1-Lipschitz functions.

    W1 = sup_{||f||_Lip <= 1} [E_p[f(x)] - E_q[f(y)]]
    For 1D: optimal f is monotone, so we test piecewise linear functions.
    """
    # For 1D, the analytical result is:
    # W1 = integral |F_p(x) - F_q(x)| dx
    # where F_p, F_q are CDFs

    # Method 1: Primal (sort and match)
    ...
    # Method 2: SciPy (uses CDF integration)
...
for shift in [0.0, 0.5, 1.0, 2.0, 5.0]:
    ...
    # KL (Gaussian closed form)
    kl = 0.5 * shift**2

    # JSD (histogram approximation)
...
print("\nKey insight:")
print("  KL: quadratic growth, explodes when supports don't overlap")
print("  JSD: saturates at log(2), gradient vanishes")
print("  W1: linear growth, ALWAYS provides meaningful gradient")
```
...
Arjovsky+ (2017) [^13] の WGAN は KR 双対性を直接実装する:

$$
\min_G \max_{f: \text{1-Lip}} \left[\mathbb{E}_{x \sim p_\text{data}}[f(x)] - \mathbb{E}_{z \sim p_z}[f(G(z))]\right]
$$

- 生成器 $G$: Wasserstein 距離を最小化
- 判別器 $f$（critic）: 1-Lipschitz 関数として Wasserstein 距離を最大化（推定）

1-Lipschitz 制約の実現方法:
- 原論文: weight clipping（$|w| \leq c$）→ 粗い近似
- WGAN-GP: 勾配ペナルティ $\lambda \mathbb{E}[(\|\nabla_{\hat{x}} f(\hat{x})\| - 1)^2]$ → 改良
- Spectral Normalization (3.11c): 各層を正規化 → 最もエレガント

> 最適輸送（OT）の完全な理論は第13回で扱う。ここでは KR 双対性が凸最適化双対性の応用であること、Lipschitz 制約と自然に結びつくことを掴んでほしい。

:::message
ここが生成モデル理論の重要な分岐点だ。KL は滑らかだがサポートが重ならないと $\infty$ に吹き飛ぶ。JSD は有界だが勾配が消失しうる。Wasserstein は常に意味のある勾配を与えるが計算が重い。どの距離を使うかで生成モデルの挙動が根本的に変わる。この比較は第12-14回（GAN）で実践的に体験する。
:::

### 3.12 ボス戦 — Cross-Entropy Loss の完全分解

さあ、ボス戦だ。LLM の学習で毎ステップ計算される Cross-Entropy Loss を、情報理論の全道具を使って完全に分解する。
...
**なぜ Cross-Entropy なのか**（理論的必然性）:

1. **MLE との等価性**: Cross-Entropy の最小化は最尤推定（MLE）と等価（第7回で証明）
2. **KL 最小化との等価性**: $H(\hat{p})$ が定数だから Cross-Entropy $\downarrow$ $\iff$ KL $\downarrow$
3. **Source Coding Theorem**: Shannon の定理 [^1] により、$H(\hat{p})$ はデータの圧縮限界。Cross-Entropy $H(\hat{p}, p_\theta) \geq H(\hat{p})$
