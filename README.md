...
数式を一行一行導出し、Python（PyTorch + Triton）で研究・機械学習訓練を実装し、Rust で推論・配布・インフラを実装する。全て無料。MacBook M1 16GB で完結する。
...
- **メインシリーズ (ml-lecture-01 ~ 50)**: 全50回の深層生成モデル講義
  ...
  - 実装は Python（PyTorch + Triton、研究・機械学習訓練）と Rust（推論・配布・インフラ）の二刀流で書く。Elixirは分散回のみRust NIF経由で登場。Colabに貼り付けて実行するだけのコードではない
...
**解決策**: 個人研究者が完全な代替を構築して無料公開する。より深い理論（全導出過程）、Python（PyTorch + Triton）で研究・機械学習訓練 + Rust（推論・配布・インフラ、ゼロコピー・短縮記法・抽象化）+ Elixir（分散回のみNIF経由）、2025-2026年のSOTAカバレッジ。GPUクラスタも¥33,000も抽選も要らない。

**規模**:
- 合計: 約183,000行の技術コンテンツ
- 200以上のarXiv引用（全件検証済み、捏造なし）
- 2,000以上の数式（全て導出過程つき）
- 1,500以上のコードブロック (Python/Rust/Elixir)
...
| 実装 | Python参照実装のみ（またはなし） | Python（PyTorch+Triton）研究実装 + Rust（推論・配布）+ Elixir（分散回のみNIF経由） |
...
「理論を学んだ」と「動くコードが書ける」は別の能力だ。VAEのELBOをPyTorchでゼロから再現できるか。逆微分の計算グラフで何が起きているか説明できるか。本シリーズは理論の直後に実装が来る。

- Python（PyTorch + Triton）: 研究・機械学習訓練（論文実装・実験・アブレーション）
- Rust: 推論・配布・インフラ（ゼロコピー・短縮記法・ゼロコスト抽象化、数式↔コード1:1コメント）
...
- 基礎的な線形代数（行列演算、固有値）
- 微積分（偏微分、連鎖律）
- Python の基本的な読み書き（Course I で PyTorch を使用）
- Rustの基本的な読み書き（Course II 以降の推論・配布実装で使用、Course II 内で習得可能）
...
| 言語 | 役割 | 採用理由 |
|:---------|:-----|:----|
| Python 🐍 | **研究・機械学習訓練** | PyTorch 2.7 + Triton 3.x による高速実験・論文実装、型ヒント+ruff+mypy による本番品質、einops による可読性の高いテンソル操作 |
| Rust 🦀 | **推論・配布・インフラ** | ゼロコピー、所有権モデルによるメモリ安全性、イテレータ短縮記法、ゼロコスト抽象化（`impl Trait`）、C-ABI FFIハブ、wgpu による GPU抽象化 |
| Elixir | 分散システム（NIF経由サブ） | BEAM VM の軽量プロセスモデル（100万プロセス）、OTPスーパービジョンによる耐障害性、Rustler による Rust NIF、Nx/Bumblebee による ML推論 |

Pythonは研究・機械学習訓練のメインスタックだ。PyTorch + Triton で論文を実装し、実験し、アブレーションを回す。Rustは推論・配布・インフラの本番スタックだ。「Pythonでしか書けない」は本番エンジニアとして限界がある。研究と本番の両方を扱えてこそ、論文から製品まで一気通貫できる。

### 主要ライブラリ

- **Python**: PyTorch 2.7, Triton 3.x, einops, numpy, ruff, mypy, FlashAttention-3
- **Rust**: ndarray, rayon, tokio, wgpu, serde, rustler, tch-rs（PyTorch C++ bindings）, ort（ONNX Runtime）
...
**技術スタック**:
- 言語: Python（PyTorch + Triton）/ Rust / Elixir
